 ...
 *
 * (C) Copyright 2015 Octav Zlatior <octav.zlatior@theobroma-systems.com>
 *
 * Based on sunxi_display.c Allwinner display driver for u-boot:
...
enum sunxi_lvds_data_mapping {
	sunxi_lvds_ns,
	sunxi_lvds_jeida,
};

struct sunxi_display_lvds_pins {
	int power_enable;
	int panel_reset;
	int bl_enable;
	int bl_pwm;
...
struct sunxi_display_pdata {
	unsigned int base;
	unsigned int depth;
	unsigned int fb_size;
	unsigned int hpd;
	unsigned int hpd_delay;
	unsigned int edid;
	unsigned int clock_phase;
	enum sunxi_lvds_data_mapping data_mapping;
	struct sunxi_display_lvds_pins pins;
...
/*
 *	when more than one device is present, this is the one being used
 *	(based on the order in the dts)
 */
static struct udevice* first_display_device = NULL;
...
static int sunxi_hdmi_hpd_detect(struct udevice *dev, int hpd_delay)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);

	struct sunxi_ccm_reg * const ccm =
		(struct sunxi_ccm_reg *)SUNXI_CCM_BASE;
	struct sunxi_hdmi_reg * const hdmi =
		(struct sunxi_hdmi_reg *)(pdata->base);
...
static void sunxi_hdmi_shutdown(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
...
static int sunxi_hdmi_ddc_do_command(struct udevice *dev,
	u32 cmnd, int offset, int n)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
...
static int sunxi_hdmi_ddc_read(struct udevice *dev,
	int offset, u8 *buf, int count)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	...
	while (count > 0) {
		...
		if (sunxi_hdmi_ddc_do_command(dev,
...
static int sunxi_hdmi_edid_get_block(struct udevice *dev, int block, u8 *buf)
{
	int r, retries = 2;

	do {
		r = sunxi_hdmi_ddc_read(dev, block * 128, buf, 128);
		if (r)
			continue;
...
static int sunxi_hdmi_edid_get_mode(
	struct udevice *dev, struct ctfb_res_modes *mode)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	...
	r = sunxi_hdmi_edid_get_block(dev, 0, (u8 *)&edid1);
	if (r == 0) {
	...
	if (r == 0) {
		...
		for (i = 0; i < ext_blocks; i++) {
			if (sunxi_hdmi_edid_get_block(dev, 1 + i,
						(u8 *)&cea681[i]) != 0) {
				ext_blocks = i;
				break;
			}
	...
	dev->driver_data = sunxi_monitor_dvi;
	for (i = 0; i < ext_blocks; i++) {
		...
		if (EDID_CEA861_SUPPORTS_BASIC_AUDIO(cea681[i]))
			dev->driver_data = sunxi_monitor_hdmi;
	}
...
static void sunxi_frontend_init(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
...
static void sunxi_frontend_mode_set(struct udevice *dev,
...
static void sunxi_frontend_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
 ...
 */
static void sunxi_composer_init(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
	//TODO: the register bases should be in the dts instead?
...
static void sunxi_composer_mode_set(struct udevice *dev,
	const struct ctfb_res_modes *mode, unsigned int address)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
...
static void sunxi_composer_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
	//TODO: the register bases should be in the dts instead?
...
static void sunxi_lcdc_init(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
	...
	struct sunxi_ccm_reg * const ccm =
		(struct sunxi_ccm_reg *)SUNXI_CCM_BASE;
//TODO: until we define all registers in the dts, we still have to use the
//platform constant here instead of using pdata
	...
	if (dev->driver_data==sunxi_monitor_lcd) {
#ifdef CONFIG_SUNXI_GEN_SUN6I
...
static void sunxi_lcdc_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
...
//platform constant here instead of using pdata
	...
	int data_mapping = SUNXI_LCDC_TCON0_LVDS_INTF_NS;
	switch (pdata->data_mapping) {
		case sunxi_lvds_ns:
			data_mapping = SUNXI_LCDC_TCON0_LVDS_INTF_NS;
			break;
		case sunxi_lvds_jeida:
			data_mapping = SUNXI_LCDC_TCON0_LVDS_INTF_JEIDA;
			break;
	}

	setbits_le32(&lcdc->ctrl, SUNXI_LCDC_CTRL_TCON_ENABLE);
	// TODO: dual mode
	setbits_le32(&lcdc->tcon0_lvds_intf,
		SUNXI_LCDC_TCON0_LVDS_INTF_ENABLE | data_mapping);
...
static void sunxi_lcdc_panel_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	 ...
	 */
	pin = pdata->pins.bl_enable;
	if (pin >= 0) {
		gpio_request(pin, "lcd_backlight_enable");
		gpio_direction_output(pin, 0);
	}

	pin = pdata->pins.bl_pwm;
	if (pin >= 0) {
		gpio_request(pin, "lcd_backlight_pwm");
		gpio_direction_output(pin, PWM_OFF);
	}

	reset_pin = pdata->pins.panel_reset;
	if (reset_pin >= 0) {
...
static void sunxi_lcdc_backlight_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	int pin;
...
static void sunxi_lcdc_tcon0_mode_set(struct udevice *dev,
...
//platform constant here instead of using pdata
	...
	if (pdata->depth == 18 || pdata->depth == 16) {
		...
		writel(((pdata->depth == 18) ?
			SUNXI_LCDC_TCON0_FRM_CTRL_RGB666 :
			SUNXI_LCDC_TCON0_FRM_CTRL_RGB565),
			   &lcdc->tcon0_frm_ctrl);
	}

	val = SUNXI_LCDC_TCON0_IO_POL_DCLK_PHASE(pdata->clock_phase);
...
static void sunxi_lcdc_tcon1_mode_set(struct udevice *dev,
					  ...
					  bool use_portd_hvsync)
{
	...
	if (dev->driver_data!=sunxi_monitor_hdmi)
		return;
...
static void sunxi_hdmi_setup_info_frames(
	struct udevice *dev, const struct ctfb_res_modes *mode)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
...
static void sunxi_hdmi_mode_set(struct udevice *dev,
	const struct ctfb_res_modes *mode, int clk_div, int clk_double)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	...
	if (dev->driver_data == sunxi_monitor_hdmi)
		sunxi_hdmi_setup_info_frames(dev, mode);
...
static void sunxi_hdmi_enable(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
...
static void sunxi_drc_init(struct udevice *dev)
{
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	(void)pdata;
	// TODO: registers from pdata
...
static void sunxi_engines_init(struct udevice *dev)
{
	sunxi_composer_init(dev);
	sunxi_lcdc_init(dev);
	sunxi_drc_init(dev);
}

static void sunxi_mode_set(struct udevice *dev,
	const struct ctfb_res_modes *mode, unsigned int address)
{
	...
	case sunxi_monitor_hdmi:
		sunxi_composer_mode_set(dev, mode, address);
		sunxi_lcdc_tcon1_mode_set(dev, mode, &clk_div, &clk_double, 0);
		sunxi_hdmi_mode_set(dev, mode, clk_div, clk_double);
		sunxi_composer_enable(dev);
		sunxi_lcdc_enable(dev);
		sunxi_hdmi_enable(dev);
		break;
	case sunxi_monitor_lcd:
		sunxi_lcdc_panel_enable(dev);
		sunxi_composer_mode_set(dev, mode, address);
		sunxi_lcdc_tcon0_mode_set(dev, mode, false);
		sunxi_composer_enable(dev);
		sunxi_lcdc_enable(dev);
		sunxi_lcdc_backlight_enable(dev);
		break;
	case sunxi_monitor_vga:
		// TODO: support vga mode
		break;
	}
}
...
/*
 *	Find the first available display device
 */
static struct udevice* sunxi_init_first_display_device(void) {
	struct udevice *root = dm_root();
	struct udevice *child = NULL;

	first_display_device = NULL;

	device_find_first_child(root, &child);
	if (child!=NULL) do {
		if (device_get_uclass_id(child)==UCLASS_DISPLAY_PORT) {
			if (device_probe(child)==0) {
				first_display_device = child;
				return child;
			}
		}
...
/*
 *	Return the device obtained via sunxi_init_first_display_device()
 */
static struct udevice* sunxi_get_first_display_device(void) {
	return first_display_device;
}

/*
 *	This function is called by the console driver to initialise output
 *	Normally, the console driver should perform these operations (?),
 *	or at least look for a list of display devices and pick one to init,
 *	but for now it will work like this for backwards compatibility
 */
void *video_hw_init(void)
{
	struct udevice *dev = sunxi_init_first_display_device();

	if (dev) {
		struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
		return &pdata->graphic_device;
	}
...
int sunxi_hdmi_hw_init(struct udevice *dev, struct ctfb_res_modes **mode) {
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	struct ctfb_res_modes *custom = malloc(sizeof(*custom));
	int ret;

	dev->driver_data = sunxi_monitor_hdmi;
	ret = sunxi_hdmi_hpd_detect(dev, pdata->hpd_delay);
	if (ret) {
		if (pdata->edid && sunxi_hdmi_edid_get_mode(dev, custom) == 0)
			*mode = custom;
	} else if (pdata->hpd) {
		sunxi_hdmi_shutdown(dev);
		return -1;
	}
...
int sunxi_lcd_hw_init(struct udevice *dev, struct ctfb_res_modes **mode) {
	dev->driver_data = sunxi_monitor_lcd;
	return 0;
}

int sunxi_dvi_hw_init(struct udevice *dev, struct ctfb_res_modes **mode) {
	dev->driver_data = sunxi_monitor_dvi;
	return 0;
}

int sunxi_vga_hw_init(struct udevice *dev, struct ctfb_res_modes **mode) {
	dev->driver_data = sunxi_monitor_vga;
	printf("VGA mode not supported by this driver\n");
	return -1;
}

int sunxi_video_hw_init(struct udevice *dev) {
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	GraphicDevice *graphic_device = &pdata->graphic_device;
	const struct ctfb_res_modes *mode = &pdata->mode;
	struct ctfb_res_modes *custom = NULL;

	int ret;

	switch (dev->driver_data) {
		case sunxi_monitor_hdmi:
			ret = sunxi_hdmi_hw_init(dev, &custom);
			break;
		case sunxi_monitor_lcd:
			ret = sunxi_lcd_hw_init(dev, &custom);
			break;
		case sunxi_monitor_dvi:
			ret = sunxi_dvi_hw_init(dev, &custom);
			break;
		case sunxi_monitor_vga:
			ret = sunxi_vga_hw_init(dev, &custom);
			break;
		default:
			ret = -1;
	};
	...
	if (mode->vmode != FB_VMODE_NONINTERLACED) {
		printf("Only non-interlaced modes supported.\n");
		ret = -1;
	}
	if (ret != -1) {
		printf("Setting up a %dx%d %s console\n", mode->xres,
			   mode->yres, sunxi_get_mon_desc(dev->driver_data));
	}

	pdata->fb_size =
		(mode->xres * mode->yres * 4 + 0xfff) & ~0xfff;
	if (pdata->fb_size > CONFIG_SUNXI_MAX_FB_SIZE) {
	...
	gd->fb_base = gd->bd->bi_dram[0].start +
			  gd->bd->bi_dram[0].size - pdata->fb_size;
	sunxi_engines_init(dev);
	sunxi_mode_set(dev, mode, gd->fb_base - CONFIG_SYS_SDRAM_BASE);
...
{
	struct udevice *dev = sunxi_get_first_display_device();
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	GraphicDevice *graphic_device = &pdata->graphic_device;
	int offset, ret;
	u64 start, size;
...
#define PIPELINE_PREFIX
	...
	size = gd->bd->bi_dram[0].size - pdata->fb_size;
...
int sunxi_display_bind(struct udevice *dev) {
	return 0;
};

int sunxi_display_ofdata_to_platdata(struct udevice *dev) {
	struct sunxi_display_pdata *pdata = dev_get_platdata(dev);
	...
	switch (dev->driver_data) {
		case sunxi_monitor_lcd: 
			ch0_node = fdt_subnode_offset(blob, node, "lvds-channel@0");
			if (ch0_node<0)
				printf("Could not locate lvds-channel node\n");
			break;
	}

	pdata->base = fdtdec_get_addr(blob, node, "reg");
	pdata->depth = fdtdec_get_int(blob, ch0_node, "data-width", 24);
	pdata->hpd = fdtdec_get_int(blob, ch0_node, "hpd", 1);
	pdata->hpd_delay = fdtdec_get_int(blob, ch0_node, "hpd_delay", 500);
	pdata->clock_phase = fdtdec_get_int(blob, node, "clock-phase", 0);
	pdata->edid = fdtdec_get_int(blob, ch0_node, "edid", 1);

	if (dev->driver_data==sunxi_monitor_lcd) {
		cell = fdt_getprop(blob, node, "data-mapping", NULL);
		if (cell) {
			if (strcmp(cell, "ns"))
				pdata->data_mapping = sunxi_lvds_ns;
			else if (strcmp(cell, "jeida"))
				pdata->data_mapping = sunxi_lvds_jeida;
			else
				debug("Invalid LVDS data mapping, falling back to NS\n");
		}
		else
			debug("Missing LVDS data mapping, falling back to NS\n");
	}

	struct display_timing timing;
	int ret = fdtdec_decode_display_timing(blob, ch0_node, 0, &timing);
	if (ret!=0) {
		printf("Could not decode display timing for sunxi display\n");
		return -1;
	}

	int ht = timing.hactive.typ + timing.hfront_porch.typ+
		 timing.hback_porch.typ + timing.hsync_len.typ;
	int vt = timing.vactive.typ + timing.vfront_porch.typ+
		 timing.vback_porch.typ + timing.vsync_len.typ;
	pdata->mode.xres = timing.hactive.typ;
	pdata->mode.yres = timing.vactive.typ;
	pdata->mode.pixclock_khz = timing.pixelclock.typ/1000;
	pdata->mode.hsync_len = timing.hsync_len.typ;
	pdata->mode.vsync_len = timing.vsync_len.typ;
	pdata->mode.left_margin = timing.hback_porch.typ - timing.hsync_len.typ;
	pdata->mode.right_margin = ht - timing.hactive.typ - timing.hback_porch.typ;
	pdata->mode.upper_margin = timing.vback_porch.typ - timing.vsync_len.typ;
	pdata->mode.lower_margin = vt - timing.vactive.typ - timing.vback_porch.typ;
	pdata->mode.sync =
		(!!(timing.flags & DISPLAY_FLAGS_HSYNC_HIGH)) * FB_SYNC_HOR_HIGH_ACT |
		(!!(timing.flags & DISPLAY_FLAGS_VSYNC_HIGH)) * FB_SYNC_VERT_HIGH_ACT;
	pdata->mode.vmode =
		(!!(timing.flags & DISPLAY_FLAGS_INTERLACED)) * FB_VMODE_INTERLACED |
		(!!(timing.flags & DISPLAY_FLAGS_DOUBLESCAN)) * FB_VMODE_DOUBLE;

	if (dev->driver_data==sunxi_monitor_lcd) {
		pdata->pins.power_enable = -1;
		pdata->pins.panel_reset = -1;
		pdata->pins.bl_enable = -1;
		pdata->pins.bl_pwm = -1;
		int pins = fdt_subnode_offset(blob, node, "pins");
		if (pins<0)
			printf("Could not locate lvds pins node\n");
		else {
			cell = fdt_getprop(blob, pins, "power-enable", NULL);
			if (cell)
				pdata->pins.power_enable = sunxi_name_to_gpio(cell);
			cell = fdt_getprop(blob, pins, "panel-reset", NULL);
			if (cell)
				pdata->pins.panel_reset = sunxi_name_to_gpio(cell);
			cell = fdt_getprop(blob, pins, "bl-enable", NULL);
			if (cell)
				pdata->pins.bl_enable = sunxi_name_to_gpio(cell);
			cell = fdt_getprop(blob, pins, "bl-pwm", NULL);
			if (cell)
				pdata->pins.bl_pwm = sunxi_name_to_gpio(cell);
		}
	}
...
int sunxi_display_probe(struct udevice *dev) {
	return sunxi_video_hw_init(dev);
};

static const struct udevice_id sunxi_display_id[] = {
	{"allwinner,sun6i-a31-ldb", sunxi_monitor_lcd},
	{"allwinner,sun6i-a31-hdmi", sunxi_monitor_hdmi},
	{"allwinner,sun6i-a31-dvi", sunxi_monitor_dvi},
	{"allwinner,sun6i-a31-vga", sunxi_monitor_vga},
	{},
};
...
U_BOOT_DRIVER(sunxi_display) = {
	...
	.of_match					= sunxi_display_id,
	.ofdata_to_platdata			= sunxi_display_ofdata_to_platdata,
	.bind  						= sunxi_display_bind,
	.probe						= sunxi_display_probe,
	.platdata_auto_alloc_size	= sizeof(struct sunxi_display_pdata),
};
