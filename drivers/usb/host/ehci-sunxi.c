...
struct ehci_sunxi_priv {
	...
	/* GPIOs to enable/power external components on the USB bus
	   (e.g. PHYs or hubs) */
	struct gpio_desc enable_gpios[4];
};
...
static int ehci_ofdata_to_platdata(struct udevice *dev)
{
	...
	ret = gpio_request_list_by_name(dev, "enable-gpios",
					priv->enable_gpios,
					ARRAY_SIZE(priv->enable_gpios),
					GPIOD_IS_OUT);
	if (ret < 0) {
		debug("%s: no enable-gpios defined\n", dev->name);
		return -ENOENT;
	}
